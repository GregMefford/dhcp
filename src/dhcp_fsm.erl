        ...
        {Si, YiAddr} ->
            case delegate(request, Pkg, State) of
                {ok, RPkg, State1} ->
                    ...
                    {next_state, bound, State#state{last=erlang:timestamp(),
                                                    handler_state = State1}, ?S(Timeout)};
                {ok, State1} ->
                    {next_state, offered, State#state{last=erlang:timestamp(),
        ...
        {ok, Reply, S1} ->
            ...
            case dhcp_package:valid_reply(R) of
                true ->
                    ...
                    {ok, R, State#state{handler_state = S1, last=erlang:timestamp()}};
                false ->
                    lager:error("[DHCP] invalid reply package for ~p:~p -> ~p", [M, F, R])
            end;
        {ok, S1} ->
            {ok, State#state{handler_state = S1, last=erlang:timestamp()}};
        E ->
            E
    end.
